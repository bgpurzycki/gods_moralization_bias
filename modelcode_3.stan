 ...
 for (d in 1:D){     //# For each deity
  for (q in 1:Q){    //# For each question
   for (i in 1:N){   //# Loop over individuals 
     ...
     //# Define model
       Xi = Beta[Site[i],q,d,1] + Beta[Site[i],q,d,2]*Age[i] + Beta[Site[i],q,d,3]*Male[i] + Beta[Site[i],q,d,4]*Education[i] + Beta[Site[i],q,d,5]*C_Shape[Site[i],Bleeding[i,q,d]]*Delta[i,q,d];
